...
from airport.models import Airplane, AirplaneType, TicketClass, Tariff, Crew, Airport, Route, Seat
from airport.serializers import AirplaneListRetrieveSerializer, TariffListRetrieveSerializer, SeatListRetrieveSerializer
...
class TariffsListTest(TestCase):
    ...
    def _create_tariff(self):
        ...
        self.tariff_for_first_class_1 = Tariff.objects.create(
            code="A",
            name="standard fare",
            ticket_class=self.ticket_class_first_class)

        self.tariff_for_first_class_2 = Tariff.objects.create(
            code="B",
            name="first class at discounted price",
            ticket_class=self.ticket_class_business_class)

        self.tariff_for_first_class_3 = Tariff.objects.create(
            code="C",
            name="superior comfort fare",
            ticket_class=self.ticket_class_first_class)

        self.tariff_for_business_class_1 = Tariff.objects.create(
            code="D",
            name="tickets at discounted price",
            ticket_class=self.ticket_class_business_class)
...
class SeatListTest(TestCase):
    ...
    def _create_seats(self):
        ...
        self.seat_1 = Seat.objects.create(airplane=self.airplane_1, seat=1, row="A",
                                          ticket_class=self.ticket_class_first_class)
        self.seat_2 = Seat.objects.create(airplane=self.airplane_1, seat=2, row="A",
                                          ticket_class=self.ticket_class_first_class)
        self.seat_3 = Seat.objects.create(airplane=self.airplane_1, seat=3, row="A",
                                          ticket_class=self.ticket_class_first_class)

        self.seat_4 = Seat.objects.create(airplane=self.airplane_1, seat=4, row="B",
                                          ticket_class=self.ticket_class_first_class)
        self.seat_5 = Seat.objects.create(airplane=self.airplane_1, seat=5, row="B",
                                          ticket_class=self.ticket_class_first_class)
        self.seat_6 = Seat.objects.create(airplane=self.airplane_1, seat=6, row="B",
                                          ticket_class=self.ticket_class_first_class)
